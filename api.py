...
— Sécurisé : rate limiting, validation URL, headers, anti-abus
"""
...
# ── CORS strict ───────────────────────────────────────────────────────────────
ALLOWED_ORIGINS = ["https://alphaconvert-web-production.up.railway.app"]
...
async def rate_limit(request: Request):
    ...
    if ip in _ban_store:
        if now < _ban_store[ip]:
            raise HTTPException(status_code=429, detail=f"Banni. Réessaie dans {int(_ban_store[ip]-now)}s.")
        del _ban_store[ip]
    _rate_store[ip] = [t for t in _rate_store[ip] if now - t < RATE_WINDOW]
    if len(_rate_store[ip]) >= BAN_AT:
        _ban_store[ip] = now + BAN_DURATION
        logger.warning(f"IP bannie: {ip}")
        raise HTTPException(status_code=429, detail="Abus détecté. Banni 5 minutes.")
    if len(_rate_store[ip]) >= RATE_LIMIT:
        raise HTTPException(status_code=429, detail="Limite atteinte (10 req/min).")
    _rate_store[ip].append(now)

# ── Validation URL ────────────────────────────────────────────────────────────
ALLOWED_DOMAINS = ["youtube.com","www.youtube.com","youtu.be","m.youtube.com",
                   "music.youtube.com","tiktok.com","www.tiktok.com","vm.tiktok.com","vt.tiktok.com"]
DANGEROUS = ["javascript:","data:","file://","ftp://","../","..\\",
             "localhost","127.0.0.1","0.0.0.0","169.254.","192.168.","10.0."]
...
async def get_info(url: str):
    ...
    try:
        ...
        logger.warning(f"yt-dlp info [{platform}] failed")
    key = _get_rapidapi_key()
    if key:
        try:
            if platform == "tiktok":
...
async def download(url: str, format: str = "mp4", quality: str = "720"):
    ...
    url = clean_url(url)
    platform = detect_platform(url)
    if format not in ("mp4","mp3"): raise HTTPException(status_code=400, detail="Format invalide.")
